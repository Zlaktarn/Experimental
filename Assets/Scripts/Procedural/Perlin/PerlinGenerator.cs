...
public class PerlinGenerator : MonoBehaviour
{
    ...
    void VisualizeGrid()
    {
        GameObject visualizationParent = Instantiate(VisualizationObject, this.transform.position, Quaternion.identity);
        ...
        for (int x = 0; x < perlinGridStepSizeX; x++)
        {
            for (int y = 0; y < perlinGridStepSizeY; y++)
            {
                ...
                if (x == 0 || x == perlinGridStepSizeX -1 ||y == 0 || y == perlinGridStepSizeY-1)
                {
                    clone = Instantiate(visualizationCube,
                    new Vector3(x, 1, y)
                ...
                else
                {
                    ...
                    if((SampleStepped(x, y) >= 0.5f))
                    {
                        clone = Instantiate(visualizationCube,
                    new Vector3(x, (int)(SampleStepped(x, y) * visualizationHeightScale), y)
                    + transform.position, transform.rotation);
